...
class MessagesController extends Controller
{
    ...
    public function index($id = null)
    {
        ...
        if ($id != null) {
            $cekRole = User::where('id', $id)->first();
            if ($cekRole) {
                if ($cekRole->role == 'admin' || $cekRole->role == 'super_admin') {
                    $id = null;
                    return Redirect::to(env('APP_URL') . '/chat');
                } else if ($cekRole->role == 'penyewa' && auth()->user()->role == 'penyewa') {
                    $id = null;
                    return Redirect::to(env('APP_URL') . '/chat');
                }
            }
    ...
    public function search(Request $request)
    {
        ...
        if (auth()->user()->role == 'penyewa') {
            $records = User::where('id', '!=', Auth::user()->id)->where('role', 'pemilik_sewa')
                ->where('name', 'LIKE', "%{$input}%")
                ->paginate($request->per_page ?? $this->perPage);
        } else {
            $records = User::where('id', '!=', Auth::user()->id)->where('role', 'penyewa')
                ->where('name', 'LIKE', "%{$input}%")
                ->paginate($request->per_page ?? $this->perPage);
        }
